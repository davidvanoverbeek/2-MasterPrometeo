...
const register = async (req, res, next) => {
    try {
        ...
        // Intentamos encontrar el usuario para no tener el mismo email o username repetido en la DB

        const userEmailExist = await User.findOne({ email: newUser.email });
        const userUserNameExist = await User.findOne({ username: newUser.username });
        ...
        // No creamos una nueva constante porque no queremis enseñar el usuario en la respuesta
        await newUser.save();
